...
/**
 * Scheduled refresh of the mirrored access directory (pg_cron or an external
 * scheduler). Protected by the same shared secret the Welcome app uses, so an
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const { syncRoleDirectory } = await import("@/lib/access.server");
        const result = await syncRoleDirectory();
